...
{
	return g_Config.m_BcFastInputs == BC_INPUTS_CLOUD && Amount() > 0.0f;
}

float CCloudInput::Amount() const
{
	return g_Config.m_BcCloudFastInputAmount / 100.0f;
}
...
{
	return g_Config.m_BcCloudFastInputOthers && Amount() > 0.0f ? 1 : 0;
}
...
void CCloudInput::ApplyOffset(const CGameClient &GameClient, int ClientId, int &Tick, float &Intra) const
{
	if(!IsActive())
		return;
	if(!GameClient.IsFastInputLocalClient(ClientId) && !g_Config.m_BcCloudFastInputOthers)
		return;
